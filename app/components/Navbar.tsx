					...
					{navLinks.map((navLink, index) => (
						<div key={index} className="first:pt-4 lg:first:pt-0">
							{navLink.subMenuLinks && navLink.subMenuLinks.length > 0 ? (
								...
								<a href={navLink.url} className="block py-3 text-md text-gray-700 hover:text-green-500 lg:px-4 lg:py-2 lg:text-base">
									{navLink.title}
								</a>
							)}
						</div>
...
const SubMenu = ({ navLink, isMobile }: { navLink: NavLink; isMobile: boolean }) => {
	...
	return (
		...
		>
			<button
				className="flex w-full items-center justify-center gap-4 py-3 text-center text-md text-gray-700 hover:text-green-500 lg:w-auto lg:flex-none lg:justify-start lg:gap-2 lg:px-4 lg:py-2 lg:text-base"
			...
			{isDropdownOpen && (
				<AnimatePresence>
					<motion.nav
						...
						className="bg-white lg:absolute lg:z-50 lg:border lg:border-gray-300 lg:shadow-lg lg:p-2 lg:[--y-close:25%]"
					>
						{navLink.subMenuLinks?.map((subMenuLink, index) => (
							<a
								key={index}
								href={subMenuLink.url}
								className="block py-3 text-center text-gray-700 hover:text-green-500 lg:px-4 lg:py-2 lg:text-left"
							>
								{subMenuLink.title}
							</a>
						))}
